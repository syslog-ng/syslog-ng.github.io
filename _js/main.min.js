...
$(function() {
    ...
    function finalizeContent(anchorId) {
        ...
        addContentTooltips();
        if (typeof updateContributionButtons === "function") updateContributionButtons();
        if (anchorId) {
    ...
    function handleNavLinkClick(event) {
        if (!event.shiftKey && !event.ctrlKey && !event.altKey && !event.metaKey) {
            ...
            if (anchorElement) {
                ...
                var forceSpa = anchorElement.classList.contains("search-help") || anchorElement.classList.contains("spa-load");
                if (url.origin === window.location.origin && anchorElement.target !== "_blank" && (forceSpa || sameCollection(url, window.location))) {
                    event.preventDefault();
                    var urlStr = url.pathname + url.hash;
                    updated = urlStr != window.location.pathname + window.location.hash;
                    history.pushState(null, null, url);
                    if (updated) updateContentFromUrl(url); else hideSearch();
                }
                event.target.blur();
            }
            if (false == updated) {
    ...
    function alterContentForTooltip(content, url, isFullPageContent, isTextTooltip) {
        ...
        if (isTextTooltip) {
            ...
            if (contribAside) contribAside.remove();
    ...
    function setTooltipPos(event, tooltipTarget, alignment, isTextTooltip) {
        ...
        var multilineUpperPart = false == isTextTooltip && tooltipTarget.getClientRects().length > 1 && mouseX > targetRect.x + targetRect.width / 2;
        var posY = (hasMastHead ? 0 : document.documentElement.scrollTop) + targetRect.top + targetRect.height / (multilineUpperPart ? 2 : 1);
        ...
        var measure = function() {
            return tooltipRenderer.getBoundingClientRect();
        };
        var tooltipRect = measure();
        var widthCap = Math.max(200, Math.min(Math.floor(viewportWidth * 2 / 3), viewportWidth - 2 * sideGap));
        var stepFactor = 1.5;
        var maxSteps = 6;
        while (tooltipRect.width < widthCap && tooltipRect.width < tooltipRect.height && maxSteps-- > 0) {
        ...
        if (contentTooltipLeft + tooltipWidth > viewportWidth - sideGap) contentTooltipLeft = viewportWidth - sideGap - tooltipWidth;
        contentTooltip.style.left = contentTooltipLeft + "px";
        var renderedHeight = tooltipRect.height;
        var anchorTopVisible = targetRect.top;
        var anchorTopAbsolute = (hasMastHead ? 0 : document.documentElement.scrollTop) + targetRect.top;
        var availableAbove = anchorTopVisible - bottomMargin - toolTipArrowHalfSize;
        var minVisibleBelow = Math.min(180, Math.max(80, renderedHeight * .5));
        var wouldScrollBelow = renderedHeight > availableBelow;
        var wouldScrollAbove = renderedHeight > availableAbove;
        var flip = availableBelow < minVisibleBelow && availableAbove > availableBelow || wouldScrollBelow && !wouldScrollAbove;
        if (flip) {
    ...
    function setTooltipZIndex() {
        ...
        if (zIndex) contentTooltip.style.zIndex = zIndex; else {
            zIndex = 5;
        }
        contentTooltip.style.zIndex = zIndex + 1;
    }
    function showTooltip(event, tooltipText, alignment, isFullPageContent, isTextTooltip) {
        if (typeof getCookie === "function" && getCookie("settings-tooltips-enabled", "true", true) === "false") return;
    ...
    function hideTooltip(withDelay) {
        function doHideTooltip() {
            if (false == shouldShowTooltip && tooltip) contentTooltip.classList.remove("visible");
            tooltipTarget = null;
    ...
    function buildFallbackTooltipHtml(element, url) {
        ...
        if (label && label !== url) html += '<div class="tooltip-fallback-title">' + esc(label) + "</div>";
        if (hostname) html += '<div class="tooltip-fallback-host">' + esc(hostname) + "</div>";
        html += '<div class="tooltip-fallback-url">' + '<a href="' + esc(url) + '" target="_blank" rel="noopener noreferrer">' + esc(url) + "</a>" + "</div>";
        html += "</div>";
        return html;
    }
    function addContentTooltips() {
        ...
        tooltipElements.forEach(function(element) {
            ...
            element.addEventListener("mouseover", function(event) {
                ...
                } else showTooltip(event, tooltipText, alignment, isFullPageContent, isTextTooltip);
            });
        });
        ...
        document.addEventListener("scroll", function(event) {
            if (elementUnderCursor == null || shouldHideTooltip(elementUnderCursor)) {
                ...
                if (tooltipToTargetRelativePosition == null) tooltipToTargetRelativePosition = relativPos; else {
                    if (false == comparePositions(relativPos, tooltipToTargetRelativePosition)) hideTooltip(true);
                }
            }
        });
    ...
    if (searchEnabled) {
        ...
        document.addEventListener("keyup", function(event) {
            if (event.keyCode === 27) {
                if ($(".initial-content").hasClass("is--hidden")) {
                    var mode = typeof getCookie === "function" ? getCookie("settings-esc-behavior", "close", true) : "close";
                    var input = $("#search");
        ...
        document.addEventListener("keydown", function(event) {
            var combo = typeof getCookie === "function" ? getCookie("settings-hotkey-search", "Ctrl+Shift+F", true) : "Ctrl+Shift+F";
        ...
        function toggleSearch(event) {
            ...
            var prefillEnabled = typeof getCookie === "function" ? getCookie("settings-prefill-selection", "true", true) === "true" : true;
            var selectedText = "";
            ...
            if ($(".initial-content").hasClass("is--hidden")) {
                setTimeout(function() {
                    ...
                    var restoreSearchScroll = typeof getCookie === "function" ? getCookie("settings-restore-search-scroll", "true", true) !== "false" : true;
                    if (restoreSearchScroll && savedSearchQuery !== null && savedSearchScrollY !== null && currentQuery === savedSearchQuery) {
...
(function() {
    ...
    function init() {
        ...
        if (restoreSearchScrollChk) restoreSearchScrollChk.checked = readBool(RESTORE_SEARCH_SCROLL_KEY, true);
        ...
        function syncDarkCheckbox() {
            if (typeof getCookie === "function") darkChk.checked = getCookie("skin-state", "light", true) === "dark";
        }
        syncDarkCheckbox();
        ...
        if (restoreSearchScrollChk) {
            restoreSearchScrollChk.addEventListener("change", function() {
                setCookie(RESTORE_SEARCH_SCROLL_KEY, restoreSearchScrollChk.checked ? "true" : "false");
            });
        }
        var recording = false;
