...
- The configuration file parser can internally stack a maximum of 20,000 token elements by default.
    ...
    ![]({{ site.baseurl}}/assets/images/warning.png) **DANGER:** Be cautious when setting a higher value for `SYSLOG_NG_CONFIG_MAX_STACK_DEPTH`, as it increases memory consumption during parsing and potentially at runtime as well (because the increased max stack depth allows larger configuration files, which likely require more memory).
    {: .notice--danger}
