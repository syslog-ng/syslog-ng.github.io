...
If syslog-ng cannot create the pipe, it aborts and produces an error. This could be caused by the lack of a write permission, or missing directory. The latter of the two can be fixed by using the `create-dirs(yes)` option.
{: .notice--primary}
...
As of {{ site.product.name }} 3.0.2, pipes are created
...
If the `fifo` file is deleted, and the user creates a new one, {{ site.product.short_name }} will still attempt to access the old file until {{ site.product.short_name }} is restarted. Make sure to restart {{ site.product.short_name }} after creating a new `fifo` file.
{: .notice--primary}
